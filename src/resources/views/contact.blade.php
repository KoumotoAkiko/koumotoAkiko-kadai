...
<div class="contact-form">
  ...
  <div class="contact-form__inner">
    <form action="confirm" method="post">
      ...
      <div class="contact-form__group contact-form__name-group">
        ...
        <div class="contact-form__error-message">
          ...
          <p class="contact-form__error-message-first_name">{{$errors->first('first_name')}}</p>
          @endif
          @if($errors->has('last_name'))
          <p class="contact-form__error-message-last_name">{{$errors->first('last_name')}}</p>
          @endif
        </div>
      </div>

    <div class="contact-form__group">
      <label class="contact-form__label">性別<span class="contact-form__required">※</span>
      </label>
    ...
    <div class="contact-form__group">
      <label class="contact-form__label" for="tel">電話番号<span class="contact-form__required">※</span>
      </label>
    ...
    <div class="contact-form__group">
      <label class="contact-form__label" for="">お問い合わせの種類<span class="contact-form__required">※</span>
    </label>
    <div class="contact-form__select-inner">
      <select class="contact-form__select" name="category_id" id="">
        <option disabled selected>選択してください</option>
        @foreach($categories as $category)
        <option value="{{$category->id}}" {{old('category_id')==$category->id ? 'select' : ''}}>{{$category->content}}</option>
        @endforeach
      </select>
    </div>
    <p class="contact-form__error-message">
    ...
    <div class="contact-form__genre">
      <label class="contact-form__genre-label" for="">興味あるものを選択してください（複数選択可）
      </label>
      @foreach($genres as $genre)
      <div class="contact-form__select-genre">
        <input class="select-genre" type="checkbox" name="genres[]" value="{{$genre->id}}" {{in_array($genre->id, old('genres',[])) ? 'checked' : ''}}>
        <label class="select-genre__label">
        {{$genre->name}}</label>
      </div>
      @endforeach
      <p class="contact-form__error-message">
    ...
    <div class="contact-form__group">
      <label class="contact-form__label" for="detail">お問い合わせ内容<span class="contact-form__required">※</span>
    </label>
